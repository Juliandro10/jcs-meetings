...
public final class BibleHtml {
    ...
    /** Painel lateral estreito (~30%) do JCS Tribuna. */
    public static String wrapTribunaPanel(
        String bodyHtml,
        String publicationCss,
        int bookNumber,
        int chapterNumber,
        ...
        sb.append("body{margin:0;padding:0;color:#ececf1;background:#16161c;");
        sb.append("font-family:Georgia,'Noto Serif','Times New Roman',serif;line-height:1.55;}");
        sb.append(".tnme-wrap{padding:0.25rem 0.65rem 1.5rem;}");
        if (publicationCss != null && publicationCss.length() > 0) {
            sb.append(publicationCss);
        }
        sb.append(".jwpub-content{color:#ececf1;font-size:0.95rem;}");
        sb.append(".jwpub-content p{margin:0 0 0.75em;}");
        sb.append(".prose-bible .vl,.prose-bible .vn,.jwpub-content .vl,.jwpub-content .vn{color:#c4b5fd;font-weight:700;}");
        sb.append(".jwpub-content sup,.jwpub-content .tnme-marker{font-size:0.58em!important;vertical-align:super!important;color:#8ec5ff!important;}");
        sb.append(".tnme-highlight{background:rgba(155,138,196,0.22);outline:2px solid rgba(196,181,232,0.55);border-radius:4px;}");
        sb.append("</style></head><body><div class=\"tnme-wrap\">");
        sb.append("<div class=\"jwpub-content prose-bible\">").append(highlightedBody).append("</div>");
        sb.append("</div>");
        if (shouldScroll) {
            ...
            sb.append("if(!el){el=document.querySelector('.tnme-highlight');}");
            sb.append("if(!el)return;");
            sb.append("var y=(el.offsetTop||0)-16;");
            sb.append("window.scrollTo(0,Math.max(0,y));}");
            sb.append("scrollToTarget();");
            sb.append("setTimeout(scrollToTarget,0);");
            sb.append("setTimeout(scrollToTarget,120);");
