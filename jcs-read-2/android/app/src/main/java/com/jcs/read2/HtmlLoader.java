...
public final class HtmlLoader {
    ...
    public interface SplitCallback {
        void onLoaded(String outlineHtml, String fileUrl, String notesHtml, boolean hasNotes);
    ...
    public static void loadSplitDocument(
        final Activity activity,
        final String weekFolder,
        final String htmlFileName,
        final String pkg,
        final ProgressBar progress,
        final SplitCallback callback) {
        if (progress != null) {
        ...
        new Thread(
                new Runnable() {
                    ...
                    public void run() {
                        try {
                            String html = prepareHtml(activity, weekFolder, htmlFileName, pkg);
                            NotesSplit.Parts parts = NotesSplit.split(html);
                            final LoadTarget target = buildLoadTarget(activity, parts.outlineHtml);
                            final String notesHtml = parts.hasNotes ? parts.notesHtml : "";
                            final boolean hasNotes = parts.hasNotes;
                            activity.runOnUiThread(
                                new Runnable() {
                                    ...
                                    public void run() {
                                        ...
                                        callback.onLoaded(
                                            target.inlineHtml, target.fileUrl, notesHtml, hasNotes);
                                    }
                                });
                            ...
                            activity.runOnUiThread(
                                new Runnable() {
                                    ...
                                    public void run() {
                                        if (activity.isFinishing()) return;
                                        if (progress != null) progress.setVisibility(View.GONE);
                                        callback.onError("Não foi possível abrir o documento.");
                                    }
                                });
                        }
                    }
