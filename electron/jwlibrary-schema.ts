/** JW Library userData.db schema v16 (Windows JW Library 2025+). */

export const JWL_USERDATA_SCHEMA_VERSION = 16;
...
function lastModifiedTriggers(table: string) {
  return [
    `CREATE TRIGGER TR_Update_LastModified_Insert_${table} AFTER INSERT ON ${table} BEGIN UPDATE LastModified SET LastModified = strftime('%Y-%m-%dT%H:%M:%SZ', 'now'); END`,
    `CREATE TRIGGER TR_Update_LastModified_Update_${table} AFTER UPDATE ON ${table} BEGIN UPDATE LastModified SET LastModified = strftime('%Y-%m-%dT%H:%M:%SZ', 'now'); END`,
    `CREATE TRIGGER TR_Update_LastModified_Delete_${table} AFTER DELETE ON ${table} BEGIN UPDATE LastModified SET LastModified = strftime('%Y-%m-%dT%H:%M:%SZ', 'now'); END`,
  ];
}

export const SCHEMA_TRIGGERS = [
