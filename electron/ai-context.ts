...
function isOutlineContext(context: AiChatContext) {
  return (
    context.contentKind === 'elder-outline' ||
    Boolean(context.sourcePub && isElderOutlinePubSymbol(context.sourcePub))
  );
}
...
export async function enrichAiContext(cacheDir: string, context: AiChatContext): Promise<AiChatContext> {
  ...
  let outlineResearchText = context.outlineResearchText;
  let speakerGuidelineText = context.speakerGuidelineText;
  let talkPrepSupportText = context.talkPrepSupportText;
  let outlineResearchCount = context.outlineResearchCount;
  let speakerGuidelinesAvailable = context.speakerGuidelinesAvailable;
  let talkPrepSupportAvailable = context.talkPrepSupportAvailable;

  if (outlineMode && context.sourcePub && context.sourceDocumentId != null) {
    if (!outlineResearchText) {
      outlineResearchText = await loadOutlineResearchText(
        cacheDir,
        context.sourcePub,
        context.sourceDocumentId,
      );
    }
    if (speakerGuidelineText == null) {
      speakerGuidelineText = await loadSpeakerGuidelineText(cacheDir);
    }
    if (talkPrepSupportText == null) {
      talkPrepSupportText = await loadTalkPrepSupportText(cacheDir, speakerGuidelineText);
    }
    speakerGuidelinesAvailable = Boolean(speakerGuidelineText);
    talkPrepSupportAvailable = Boolean(talkPrepSupportText);
    if (outlineResearchCount == null && outlineResearchText) {
      outlineResearchCount = (outlineResearchText.match(/^### /gm) ?? []).length;
    }
  }
  ...
  return {
    ...
    preparedOutlineText,
    outlineResearchText,
    speakerGuidelineText,
    talkPrepSupportText,
    outlineResearchCount,
    speakerGuidelinesAvailable,
    talkPrepSupportAvailable,
  };
}
